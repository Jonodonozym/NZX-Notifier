...
class ANZWebApiImpl implements ANZWebApi {
	private final DateTimeFormatter lastTradedFormatter = new DateTimeFormatterBuilder().appendPattern("dd/MM HH:mm")
			.parseDefaulting(ChronoField.YEAR, Year.now().getValue()).parseDefaulting(ChronoField.SECOND_OF_MINUTE, 0)
	...
	public boolean login(String username, String password) {
		try {
			...
			loginSession = con.data("username", username).data("password", password).data("__VIEWSTATE", __VIEWSTATE)
					.data("LoginStartIn1:ddlStartin", "../secure/accounts.aspx?view=bal").data("btnSignon", "Login")
	...
	public boolean isLoggedIn() {
		if (loginSession != null && !loginSession.url().toString().startsWith(Websites.ANZloginURL)) {
			try {
				Connection con = Jsoup.connect(Websites.ANZprefURL).cookies(loginSession.cookies());
	...
	public LocalDateTime getDateTime() {
		return NZXWebApi.getInstance().getDateTime();
	}

	@Override
	public float getSecPrice(String securityCode) {
		try {
			Document doc = Jsoup.connect(Websites.ANZquoteURL.replace(Websites.ANZquoteToReplace, securityCode))
					.cookies(loginSession.cookies()).get();
	...
	public TradeOverview getOverview(String securityCode) {
		try {
			if (isLoggedIn()) {
				...
				String[] cells = new String[] { overviewTable.select("span[id=quotelast]").text(),
	...
	private String setDefaultView(ANZView view) {
		try {
			...
			con.data("__VIEWSTATE", __VIEWSTATE).data("ddlDefaultQuoteView", view.toString())
					.data("CorporateActionPaymentCurrency", DefaultCurrency).data("btnGo", "Save").method(Method.POST)
					.execute();
