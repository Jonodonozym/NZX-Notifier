...
public final class StringEncryptor {
    ...
    private static final SecretKeySpec createSecretKey(char[] keyPartA, byte[] keyPartB, int iterationCount, int keyLength) {
    	try {
    ...
    public static final String encrypt(String string) throws GeneralSecurityException, UnsupportedEncodingException {
        ...
        pbeCipher.init(Cipher.ENCRYPT_MODE, chiperKey);
    ...
    public static final String decrypt(String string) throws GeneralSecurityException, IOException {
        ...
        Cipher pbeCipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
        pbeCipher.init(Cipher.DECRYPT_MODE, chiperKey, new IvParameterSpec(base64Decode(iv)));
    ...
    private static final byte[] xorWithKey(byte[] in, byte[] key) {
        Random r = new Random(13487139487L); // why encrypt every character? why not do it at random with a 75% chance!
        byte[] out = new byte[in.length];
        for (int i = 0; i < in.length; i++)
            out[i] = r.nextBoolean() || r.nextBoolean()?(byte) (in[i] ^ key[i%key.length]):in[i];
        return out;
    }
}
